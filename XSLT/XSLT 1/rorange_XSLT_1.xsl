...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="poemTitle">     <!-- rorange #1 -->
        <title>
            <xsl:apply-templates/>   
        </title> 
    </xsl:template> 
    ...
    <xsl:template match="body">           <!-- rorange #2 -->
            <xsl:apply-templates/>   
    </xsl:template>
    ...
    <xsl:template match="line">
        <line n="{preceding-sibling::line=>count()+1}"/>     <!-- rorange #3 and #4 -->
            <xsl:apply-templates/>     
    </xsl:template> 
    
    
    <xsl:template match="poem">
        <poem xml:id="p-{count(preceding-sibling::poem) + 1}">    <!-- rorange #5 -->
            <xsl:apply-templates/>
        </poem>
    </xsl:template>
